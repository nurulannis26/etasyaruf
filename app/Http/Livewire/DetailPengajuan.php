...
class DetailPengajuan extends Component
{
    ...
    public $etasyaruf;
    public $siftnu;
    public $jumlah_penerima_disetujui;
    // tab-a
    public $rencana_a;
    public $rencana_b;
    public $rencana_c;
    ...
    {
        $datasz = PengajuanDetail::where('id_pengajuan', $this->id_pengajuan)->where('pencairan_status', 'Berhasil Dicairkan')->get();
        
        foreach ($datasz as $a) {
            $id_pengajuan_details =  $a->id_pengajuan_detail;
        }
        ...
        $cek = PengajuanDetail::where('id_pengajuan', $this->id_pengajuan)
        ->join($this->etasyaruf . '.lampiran_berita', $this->etasyaruf . '.lampiran_berita.id_pengajuan_detail', '=', $this->etasyaruf . '.pengajuan_detail.id_pengajuan_detail')
        ->count();
       ...
       return view('livewire.detail-pengajuan', compact('rekening', 'ids', 'detail_jurnal', 'get_detail', 'cek'));
    }
    ...
    {
        $a = str_replace('.', '', $this->jumlah_penerima_disetujui ?? null);
        $b = str_replace('.', '', $this->satuan_pencairan);
        $this->nominal_pencairan = number_format((int)$a * (int)$b, 0, '.', '.');
    }
    ...
    public function detail_rencana($id)
    {
        ...
        if ($this->data_detail->approval_status == 'Disetujui') {
           ...
           if ( $this->data_detail->jumlah_penerima_disetujui == null) {
                $this->jumlah_penerima_disetujui = $this->data_detail->jumlah_penerima;
            } else {
                $this->jumlah_penerima_disetujui = $this->data_detail->jumlah_penerima_disetujui;
            }
    ...
    public function modal_edit_rencana($id)
    {
        ...
        $this->pengajuan_note = $this->data_detail->pengajuan_note;
    ...
    public function delete_penerima($id)
    {
     ...
     public function delete_semua_penerima($id)
    {
        // $this->refresh();
        $file =  PengajuanPenerimaLPJ::where('id_pengajuan_detail', $id)->get();

        foreach ($file as $f) {
            LampiranPenerimaLPJ::where('id_penerima_lpj', $f->id_pengajuan_penerima)->delete();
        }

        PengajuanPenerimaLPJ::where('id_pengajuan_detail', $id)->delete();
    ...
    public function batalRespon()
    {
        ...
        $this->emit('waktu_alert');
        $this->emit('pencairan');
        session()->flash('pencairan', 'Pencairan Rencana Pentasyarufan Berhasil Dibatalkan');
    }

   
    public function pencairan()
    {
        $datas = PengajuanDetail::where('id_pengajuan_detail', $this->id_pengajuan_detail)->update([
            'id_rekening' => $this->id_rekening,
            'tgl_pencairan' => date('Y-m-d'),
            'pencairan_status' => 'Berhasil Dicairkan',
            'staf_keuangan_pc' => Auth::user()->gocap_id_pc_pengurus,
            'satuan_pencairan' => str_replace('.', '', $this->satuan_pencairan),
            'nominal_pencairan' => str_replace('.', '', $this->nominal_pencairan),
            'pencairan_note' => $this->pencairan_note,
            'jumlah_penerima_disetujui' => $this->jumlah_penerima_disetujui,
        ]);
    ...
    public function uploadLampiranBerita()
{
    ...
    // Store the file using the specified path and file name
    $this->file->storeAs('lampiran_berita', $file_name);

    // Create the LampiranBerita record with the generated file name
    ...
    // Emit events and flash messages
    $this->emit('waktu_alert');
    session()->flash('success', 'File Berhasil di-Upload');
    ...
    public function deleteLampiranBerita()
    {
        ...
        $path = public_path() . "/uploads/lampiran_berita/" . $this->file_berita;
        // dd($path);
        if (file_exists($path)) {
            unlink($path);
        }
    ...
    public function uploadKonfirmasi()
    {
        ...
        $this->notif(
            Helper::getNohpByIdJabatan('pc', 'e7fc67fe-725b-11ed-ad27-e4a8df91d8b3'),
           // '081578447350',
           "Assalamualaikum Warahmatullahi Wabarakatuh" . "\n" . "\n" .
               "Yth. " . "*" . Helper::getNamaPengurusByIdJabatan('pc', $this->data->id_pc, 'e7fc67fe-725b-11ed-ad27-e4a8df91d8b3') .  "*" . "\n" .
               Helper::getJabatanPengurusByIdJabatan('pc', 'e7fc67fe-725b-11ed-ad27-e4a8df91d8b3')  . "\n" . "\n" .
               "*" .  "Mohon segera direspon"  . "*" .  "\n" .
               "Pengajuan Pentasyarufan Tingkat " . $tingkat . ", " . "*" . "telah dikonfirmasi" . "*" . " dengan detail sebagai berikut :" . "\n" . "\n" .
               "*" .  "Nomor"  . "*" .  "\n" .
               $this->data->nomor_surat  . "\n" .
    ...
    {
        //   dd('s');
        // dd(now()->format('Y-m-d') . $this->satuan_pencairan);
        ...
        $this->nomorr = 'JURNAL/' . $month . '/' . $year . '/' . $nomor;
        ...
        foreach ($data as $a) {
            ...
            $pilar = Helper::getDataPilar($a->id_program_pilar)->pluck('pilar')->first();
            $nama_kegiatan = Helper::getDataKegiatan($a->id_program_kegiatan)->pluck('nama_program')->first();
            $keterangan = $a->pengajuan_note;

            //PENGUATAN KELEMBAGAAN
            if ($a->id_program == 'ba84d782-81a8-11ed-b4ef-dc215c5aad51') {
                $akundb = '4250a6a0-2560-4d4b-94bb-b0d16f0a43bb';
            }
            //PROGRAM SOSIAL
            elseif ($a->id_program == 'bed10d9c-81a8-11ed-b4ef-dc215c5aad51') {
                $akundb = 'a13e0322-7995-4f11-88f4-f51f862bcc57';
            }
            //OPERASIONAL UPZIS
            elseif ($a->id_program == 'c51700b1-81a8-11ed-b4ef-dc215c5aad51') {
                $akundb = '40b7c6f8-96ee-4104-82fd-a7c39b1f3bd1';
            }

            // dd('[ '.$pilar.']' . ' - ' . $nama_kegiatan . ' - '. $keterangan);
            ...
            JurnalUmum::create([
                ...
                'id_pengajuan_detail' => $a->id_pengajuan_detail,
                'tgl_transaksi' => now()->format('Y-m-d'),
                'nomor' => $nomor,
                'no_urut' => $nomor_urut,
                'jenis' => 'Keluar',
                'akun' => 'd38525ee-416c-442b-bbad-679d861f032a',
                'bank' => $a->id_rekening,
                'deskripsi' => '[ ' . $pilar . ']' . ' - ' . $nama_kegiatan . ' - ' . $keterangan,
                'debit' => 0,
                'kredit' => is_numeric(str_replace(',', '.', str_replace('.', '', $a->nominal_pencairan))) ? str_replace(',', '.', str_replace('.', '', $a->nominal_pencairan)) : 0,
            ...
            JurnalUmum::create([
                ...
                'bank' => $a->id_rekening,
                'deskripsi' => '[ ' . $pilar . ']' . ' - ' . $nama_kegiatan . ' - ' . $keterangan,
                'debit' => is_numeric(str_replace(',', '.', str_replace('.', '', $a->nominal_pencairan))) ? str_replace(',', '.', str_replace('.', '', $a->nominal_pencairan)) : 0,
                'kredit' => 0,
                'id_pengguna' => Auth::user()->id_pengguna,
            ]);
        }
        ...
        if ($this->data->tingkat == 'Upzis MWCNU') {
            $nama_pj = Helper::getNamaPengurus('upzis', $this->data->pj_upzis);
            $tingkat = 'UPZIS MWCNU ' . Helper::getNamaUpzis($this->data->id_upzis);
            $kelembagaan = $this->sumTotalByProgram('bmt', $this->data->id_pengajuan, 'LEMBAGA');
            $sosial = $this->sumTotalByProgram('bmt', $this->data->id_pengajuan, 'SOSIAL');
            $operasional = $this->sumTotalByProgram('bmt', $this->data->id_pengajuan, 'OPERASIONAL');
            $kelembagaan2 = $this->sumTotalByProgram('bri', $this->data->id_pengajuan, 'LEMBAGA');
            $sosial2 = $this->sumTotalByProgram('bri', $this->data->id_pengajuan, 'SOSIAL');
            $operasional2 = $this->sumTotalByProgram('bri', $this->data->id_pengajuan, 'OPS');
            ...
            $nama_pj = Helper::getNamaPengurus('ranting', $this->data->pj_ranting);
            $tingkat = 'PRNU ' . Helper::getNamaRanting($this->data->id_ranting);
            $kelembagaan = $this->sumTotalByProgram2('bmt', $this->data->id_pengajuan, 'ba84d782-81a8-11ed-b4ef-dc215c5aad51');
            $sosial = $this->sumTotalByProgram2('bmt', $this->data->id_pengajuan, 'bed10d9c-81a8-11ed-b4ef-dc215c5aad51');
            $kelembagaan2 = $this->sumTotalByProgram('bri', $this->data->id_pengajuan, 'LEMBAGA');
            $sosial2 = $this->sumTotalByProgram('bri', $this->data->id_pengajuan, 'SOSIAL');
    ...
    {
        Pengajuan::where('id_pengajuan', $this->id_pengajuan)->update([
            'tgl_tolak_rekomendasi' => date('Y-m-d'),
        ...
        $no_admin_upzis = Helper::getNohpByIdJabatanUpzis('upzis', $this->data->id_upzis, 'c2a4bbd8-85c2-11ed-a0ac-040300000000');
        $this->notif(
            $no_admin_upzis,
            // '082138603051',
            "Assalamualaikum Warahmatullahi Wabarakatuh" . "\n" . "\n" .
                "Yth. " . "*" . Helper::getNamaPengurusByIdJabatan('upzis', $this->data->id_upzis, 'c2a4bbd8-85c2-11ed-a0ac-040300000000') .  "*" . "\n" .
                Helper::getJabatanPengurusByIdJabatan('upzis', 'c2a4bbd8-85c2-11ed-a0ac-040300000000')  . "\n" . "\n" .

                "Pengajuan " . "*" . "ditolak" . "*" . " lembar rekomendasi tidak dapat diterbitkan untuk Pengajuan Pentasyarufan Tingkat " . $tingkat .  ", dengan detail sebagai berikut :" . "\n" . "\n" .
    ...
    public function batalRekomendasi()
    {
        ...
        foreach ($data as $a) {
            ...
            JurnalUmum::where('id_pengajuan_detail', $a->id_pengajuan_detail)->delete();
        }
        $this->mount();
    }


    public function batalTolakRekomendasi()
    {
        Pengajuan::where('id_pengajuan', $this->id_pengajuan)->update([
            'tgl_terbit_rekomendasi' => NULL,
            'status_rekomendasi' => 'Belum Terbit',
    ...
    public function penerimaValidation()
    {
        ...
        if ($this->penerima_manfaat and $this->tgl_bantuan and $this->alamat_penerima and $this->penerima_note) {
            return 1;
        } else {
            return 0;
        }
    ...
    public function kirimNotifWa()
    {
        ...
        if ($this->data->tingkat == 'Upzis MWCNU') {
            ...
            $this->dispatchBrowserEvent('success', [
                'message' => 'Notifikasi Belum LPJ Terkirim ke <strong><br>' . Helper::getNamaPengurus('upzis', $this->data->maker_tingkat_upzis) . '</strong><br>' . Helper::getNohpPengurus('upzis', $this->data->maker_tingkat_upzis)
            ]);
        } else {
            $tingkat = 'Ranting NU ' . Helper::getNamaUpzis($this->data->id_upzis);
            $belum = PengajuanDetail::where('id_pengajuan', $this->id_pengajuan)->where('status_berita', 'Belum Dikonfirmasi')->count();
