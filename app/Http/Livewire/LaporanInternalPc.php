...
class LaporanInternalPc extends Component
{
    // database
    public $gocap;
    public $etasyaruf;
    ...
    //   dd('dw');
        ...
        $this->tahun_pengajuan = Internal::
            // filter upzis
            selectRaw('YEAR(approval_date) as tahun')->groupBy('tahun')->orderBy('approval_date', 'ASC')->get();
        ...
        $data = Internal::orderBy('created_at', 'DESC')
            ...
            ->when($filter_daterange != '', function ($query) use ($start_date, $end_date) {
                ...
                return $query->when($start_date && $end_date, function ($query) use ($start_date, $end_date) {
                    if ($start_date == $end_date) {
                        return $query->whereDate('approval_date', '=', $start_date);
                    } else {
                        return $query->whereDate('approval_date', '>=', $start_date)
                            ->whereDate('approval_date', '<=', $end_date);
                    }
                });
            })
            ...
            // cari
            // ->when($this->cari, function ($query) use ($start_date, $end_date) {
            //     return $query->where(function ($subQuery) use ($start_date, $end_date) {
            //         $subQuery->where('nomor_surat', 'like', '%' . $this->cari . '%');

            //         if ($start_date && $end_date) {
            //             if ($start_date == $end_date) {
            //                 $subQuery->whereDate('approval_date', '=', $start_date);
            //             } else {
            //                 $subQuery->whereDate('approval_date', '>=', $start_date)
            //                     ->whereDate('approval_date', '<=', $end_date);
            //             }
        ...
        return view('livewire.laporan-internal-pc', compact(
            'data',
            'pengajuan_total',
            'total_nominal',
