...
class PengajuanController extends Controller
{
    ...
    public function pdf_umum_upzis($role, $status, $start_date, $end_date, $filter_daterange, $id_upzis, $id_upzis2, $id_ranting2, $status2, $filter_daterange2)
    {
        ...
        $program = PengajuanDetail::join($this->etasyaruf . '.program_pilar', $this->etasyaruf . '.program_pilar.id_program_pilar', '=', $this->etasyaruf . '.pengajuan_detail.id_program_pilar')
            ...
            ->when($start_date && $end_date and $start_date != $end_date, function ($query) use ($start_date, $end_date) {
                return $query->whereDate('pengajuan_detail.created_at', '>=', $start_date)->whereDate('pengajuan_detail.created_at', '<=', $end_date);
            })
            ->when($start_date && $end_date and $start_date == $end_date, function ($query) use ($start_date, $end_date) {
        ...
        $sum_pencairan = PengajuanDetail::join($this->etasyaruf . '.program_pilar', $this->etasyaruf . '.program_pilar.id_program_pilar', '=', $this->etasyaruf . '.pengajuan_detail.id_program_pilar')
            ->join($this->etasyaruf . '.pengajuan', $this->etasyaruf . '.pengajuan.id_pengajuan', '=', $this->etasyaruf . '.pengajuan_detail.id_pengajuan')
            ->join($this->etasyaruf . '.program_kegiatan', $this->etasyaruf . '.program_kegiatan.id_program_kegiatan', '=', $this->etasyaruf . '.pengajuan_detail.id_program_kegiatan')
            ->select(
                $this->etasyaruf . '.pengajuan_detail.*',
        ...
        $sum_penerima = PengajuanDetail::join($this->etasyaruf . '.program_pilar', $this->etasyaruf . '.program_pilar.id_program_pilar', '=', $this->etasyaruf . '.pengajuan_detail.id_program_pilar')
            ->join($this->etasyaruf . '.pengajuan', $this->etasyaruf . '.pengajuan.id_pengajuan', '=', $this->etasyaruf . '.pengajuan_detail.id_pengajuan')
            ->join($this->etasyaruf . '.program_kegiatan', $this->etasyaruf . '.program_kegiatan.id_program_kegiatan', '=', $this->etasyaruf . '.pengajuan_detail.id_program_kegiatan')
            ->select(
                $this->etasyaruf . '.pengajuan_detail.*',
        ...
        // dd($program);
        $pdf = Dompdf::loadview('print.pdf_umum_upzis',   compact(
            'program',
            'id_upzis',
            'filter_daterange',
            'sum_pencairan',
        ...
        return $pdf->stream()->withHeaders([
            ...
            'Content-Disposition' => 'filename="REALISASI PENTASYARUFAN TINGKAT UPZIS BERDASARKAN PERSETUJUAN DIREKTUR DAN DIV. KEUANGAN' . $filter_daterange . '_pdf"',

        ]);
    }


    public function pdf_umum_ranting($role, $status, $start_date, $end_date, $filter_daterange, $id_upzis, $id_upzis2, $id_ranting2, $status2, $filter_daterange2)
    {
        // dd($request->all());
        $title = "DATA PENGAJUAN";
        // tabbed
        ...
        $datas2 = PengajuanDetail::join($this->etasyaruf . '.program_pilar', $this->etasyaruf . '.program_pilar.id_program_pilar', '=', $this->etasyaruf . '.pengajuan_detail.id_program_pilar')
            ...
            ->when($start_date2 && $end_date2 and $start_date2 != $end_date2, function ($query) use ($start_date2, $end_date2) {
                return $query->whereDate('pengajuan_detail.created_at', '>=', $start_date2)->whereDate('pengajuan_detail.created_at', '<=', $end_date2);
            })
            ->when($start_date2 && $end_date2 and $start_date2 == $end_date2, function ($query) use ($start_date2, $end_date2) {
        ...
        $pdf = Dompdf::loadview('print.pdf_umum_ranting',   compact(
            'title',
            'daftar_upzis',
            'daftar_ranting2',
            'tab_upzis',
        ...
        return $pdf->stream()->withHeaders([
            ...
            'Content-Type' => 'application/pdf',
            'Cache-Control' => 'no-store, no-cache',
            'Content-Disposition' => 'filename="REALISASI PENTASYARUFAN TINGKAT RANTING BERDASARKAN PERSETUJUAN DIREKTUR DAN DIV. KEUANGAN' . $filter_daterange2 . '_pdf"',
        ]);
    }


    public function excel_umum_ranting($role, $status, $start_date, $end_date, $filter_daterange, $id_upzis, $id_upzis2, $id_ranting2, $status2, $filter_daterange2)
    {
        ...
        $sheet->setCellValue('B5', 'NAMA PROGRAM');
        $sheet->setCellValue('C5', 'SUMBER DANA');
        $sheet->setCellValue('D5', 'TGL KONFIRMASI');
        $sheet->setCellValue('E5', 'NOMINAL PENGAJUAN ');
        $sheet->setCellValue('F5', 'TGL TERBIT REKOMENDASI');
        $sheet->setCellValue('G5', 'NOMINAL PENCAIRAN');
        $sheet->setCellValue('H5', 'PENERIMA MANFAAT');
        ...
        $sheet->getStyle('A5:H5')->applyFromArray($borderStyle); // Apply border style to header row
        $lastRow = $sheet->getHighestRow();
        $sheet->getStyle('A5:H' . $lastRow)->applyFromArray($borderStyle); // Apply border style to the rest of the rows
        ...
        if (count($program) > 0) {
            ...
            foreach ($program as $pilar => $details) {
                ...
                $sheet->setCellValue('E' . $ur, 'Rp' . number_format($jumlah_nominal_pengajuan, 0, '.', '.') . ',-');
                $sheet->setCellValue('G' . $ur, 'Rp' . number_format($jumlah_nominal_pencairan, 0, '.', '.') . ',-');
                ...
                foreach ($uniquePrograms as $a) {
                    ...
                    $sheet->setCellValue('E' . $ur, 'Rp' . number_format($details->where('nama_program', $a->nama_program)->sum('nominal_pengajuan'), 0, '.', '.') . ',-');
                    $sheet->setCellValue('G' . $ur, 'Rp' . number_format($details->where('nama_program', $a->nama_program)->sum('nominal_pencairan'), 0, '.', '.') . ',-');
                    ...
                    foreach ($details->where('nama_program', $a->nama_program) as $b) {
                        ...
                        $sheet->setCellValue('C' . $ur, strtoupper(PengajuanController::get_nama_upzis($b->id_upzis)) . ' - ' . PengajuanController::get_nama_bmt($b->id_rekening) . ' - ' . PengajuanController::no_rekening($b->id_rekening));
                        ...
                        $sheet->setCellValue('E' . $ur, 'Rp ' . number_format($b->nominal_pengajuan, 0, '.', '.') . ',-');
                        $sheet->setCellValue('F' . $ur, Carbon::parse($tgl_terbit_rekomendasi)->isoFormat('D MMMM Y'));
                        $sheet->setCellValue('G' . $ur, 'Rp ' . number_format($b->nominal_pencairan, 0, '.', '.') . ',-');
    ...
    public function excel_umum_upzis($role, $status, $start_date, $end_date, $filter_daterange, $id_upzis, $id_upzis2, $id_ranting2, $status2, $filter_daterange2)
    {
