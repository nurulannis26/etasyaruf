...
class Helper extends Controller
{
    ...
    public static function getNikPengurus($role, $id)
    {
        $siftnu = config('app.database_siftnu');
        $gocap = config('app.database_gocap');
        $a = DB::table($siftnu . '.pengguna')
    ...
    public static function getAlamatPengurus($role, $id)
    {
        ...
        $a = DB::table($siftnu . '.pengguna')
            ->where('gocap_id_' . $role . '_pengurus', $id)
            ->first();
        $alamat = $a->alamat . " RT." . $a->rt . " RW." . $a->rw;
        return $alamat ?? NULL;
    }
    ...
    public static function getNohpByIdJabatan($role, $id)
    {
      ...
      public static function getNohpByIdJabatanUpzis($role, $id_upzis_ranting, $id_jabatan)
    {
        $gocap = config('app.database_gocap');
        $siftnu = config('app.database_siftnu');
        $a = DB::table($gocap . '.pengurus_jabatan')
    ...
    public static function getFirstRekening($id_pengajuan)
    {
        $gocap = config('app.database_gocap');
        ...
        $b = null; // Inisialisasi $b di luar loop

        foreach ($a as $itemA) {
            $id_rekening = $itemA->id_rekening;
        
            $b = Rekening::where('id_rekening', $id_rekening)
                ->join($gocap . '.bmt', $gocap . '.bmt.id_bmt', '=', $gocap . '.rekening.id_bmt')
                ->where('bmt.nama_bmt', 'like', '%BMT%')
                ->first();
        
            // Jika $b ditemukan, keluar dari loop
            if ($b !== null) {
                break;
            }
