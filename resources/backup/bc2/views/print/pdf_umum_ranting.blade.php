...
<p style="margin:0pt; text-align:center; font-size:11pt;">
    <b>REALISASI PENTASYARUFAN TINGKAT RANTING</b>
    <br>
    <b>BERDASARKAN PERSETUJUAN DIREKTUR DAN DIV. KEUANGAN</b>
</p>
...
<table style="width: 100%; font-size: 11pt;">
    {{-- paragraf 1 --}}
    <tr>
        <td style="width: 19%"><b>Periode Penyaluran</b></td>
        <td style="width: 1%">:</td>
        <td style="width: 80%">
            {{ $startDate }} - {{ $endDate }}
        </td>
                ...
                @foreach ($uniquePrograms as $a)
                    ...
                    @foreach ($details->where('nama_program', $a->nama_program) as $b)
                        <tr style=" page-break-inside: avoid;">
                            ...
                            <td style="width: 18%;vertical-align:middle; border: 1px solid black; text-align:center;"><b>{{ strtoupper(app\Http\Controllers\PengajuanController::get_nama_ranting($b->id_ranting)) }}</b><br>{{ app\Http\Controllers\PengajuanController::get_nama_bmt($b->id_rekening) }}<br>({{ app\Http\Controllers\PengajuanController::no_rekening($b->id_rekening) }})
                            </td>
                                  @php
