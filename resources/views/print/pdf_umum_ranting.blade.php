    ...
    @if($sub == 'pengajuan')
        ...
        <p style="margin:0pt; text-align:center; font-size:11pt;">
            <b>DATA PENGAJUAN PENTASYARUFAN {{ $tl }}</b>
            <br>
    ...
    @elseif($sub == 'laporan')
        <p style="margin:0pt; text-align:center; font-size:11pt;">
            ...
            <b>DATA REALISASI PENTASYARUFAN BY PILAR & PROGRAM PER WILAYAH KECAMATAN {{ strtoupper(app\Http\Controllers\PengajuanController::get_nama_upzis($id_upzis2)) }} </b>
            @elseif($tings == 'ranting')
            <b>DATA REALISASI PENTASYARUFAN BY PILAR & PROGRAM TINGKAT RANTING  </b>
            @elseif($tings == 'upzis')
            <b>DATA REALISASI PENTASYARUFAN BY PILAR & PROGRAM TINGKAT UPZIS </b>
            @endif
...
<table cellpadding="2" cellspacing="0" style="width:100%;border-collapse:collapse;font-size:10pt;">
    ...
    <thead>
        <tr style="text-align:center; background-color:#cbf2d6; border: 1px solid black;">
            ...
            <td style="width: 13%;vertical-align:middle; border: 1px solid black;"><b>TGL <br> REKOM</b></td>
                ...
                @foreach ($uniquePrograms as $a)
                    ...
                    @foreach ($details->where('nama_program', $a->nama_program) as $b)
                        <tr style=" page-break-inside: avoid;">
                           ...
                           @if($b->id_ranting)
                            <td style="width: 18%;vertical-align:middle; border: 1px solid black; text-align:center;"><b>RTG {{ strtoupper(app\Http\Controllers\PengajuanController::get_nama_ranting($b->id_ranting)) }}</b><br>{{ app\Http\Controllers\PengajuanController::get_nama_bmt($b->id_rekening) }}<br>({{ app\Http\Controllers\PengajuanController::no_rekening($b->id_rekening) }})
                            </td>
                            @else
                            <td style="width: 18%;vertical-align:middle; border: 1px solid black; text-align:center;"><b>UPZ {{ strtoupper(app\Http\Controllers\PengajuanController::get_nama_upzis($b->id_upzis)) }}</b><br>{{ app\Http\Controllers\PengajuanController::get_nama_bmt($b->id_rekening) }}<br>({{ app\Http\Controllers\PengajuanController::no_rekening($b->id_rekening) }})
                            </td>
                            @endif
                                  @php
